...
\begin{center}{\huge{\textbf{Analysis 2 Cheat Sheet}}}\\
\end{center}
\begin{multicols*}{3}
...
\node [mybox] (box){%
    ...
    $\mathcal{L}^p(\Omega, \mathcal{A}, \mu) :=$\\$ \{f: \Omega \to \K | \text{f ist messbar}, \int_\Omega |f(x)|^p d\mu(x) < \infty\}$
...
\node [mybox] (box){%
    ...
    \begin{enumerate}
        ...
        \item \href{https://math.stackexchange.com/questions/617195/show-that-c0-1-is-not-a-banach-space-with-the-given-norm}{Kein Banachraum}
...
\node [mybox] (box){%
    ...
    \begin{enumerate}
        ...
        $\vect x^k \xrightarrow[]{k \to \infty} \vect x^0 \implies f(\vect x^k) \xrightarrow[]{k \to \infty} f(\vect x^0)$
    \end{enumerate}

    \end{minipage}
};
%------------ Grenzwerte und Stetigkeit von Funktionen Sätze Header ---------------------
\node[fill = blue, text=white, font=\bfseries, right=10pt] at (box.north west) {Folgenkriterium};
\end{tikzpicture}
...
\node[fill = blue, text=white, font=\bfseries, right=10pt] at (box.north west) {Sätze für Koordinatenfunktionen und Proj.};
\end{tikzpicture}
...
\node[fill = blue, text=white, font=\bfseries, right=10pt] at (box.north west) {Verknüpfungen von Funktionen};
\end{tikzpicture}
...
%------------ Monome und Polynome Satz ---------------
\begin{tikzpicture}
\node [mybox] (box){%
    \begin{minipage}{0.3\textwidth}
    Jedes Polynom ist stetig; jede rationale Fkt. $\frac{P}{Q}$ ist stetig in $\vect x \in \K^n$, wenn $Q(\vect x) \neq 0$.
    \end{minipage}
};
%------------ Monome und Polynome Satz Header ---------------------
\node[fill = blue, text=white, font=\bfseries, right=10pt] at (box.north west) {Monome/Polynome Stetigkeit};
\end{tikzpicture}
